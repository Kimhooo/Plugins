...
import com.kimho.plugin.util.ImageUtil
...
//                println("--- ${file.absolutePath}")
        ...
        if (imageFileList.size < coreNum) {
            ...
            for (f in results) {
                try {
                    ...
                    println("kimhoPlugin Convert2WebpTask#dispatchOptimizeTask() execute wrong.")
                }
            }
        }
    }
    ...
    private fun calcNewSize(path: String) {
        if (File(path).exists()) {
            ...
            if (File(webpPath).exists()) {
                ...
                println("kimhoPlugin Convert2Webp Task was wrong.")
            }
        }
    }
    ...
    private fun checkCwebpTools() {
        ...
        if (!WebpToolBean.getToolsDir().exists()) {
            throw GradleException("kimhoPlugin 'convert2Webp' task need cwebp tool.")
        }
    }
